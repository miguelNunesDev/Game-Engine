...
export class Handler extends Entity {
    ...
    getShape(shape: String) {
        const formType = {
            circle: (): Circle => { return new Circle(this.transform.center, this.transform.size.w * 0.5, this) },
            square: (): Square => { return new Square(this.transform.position, this.transform.size, this, this.stroke, 'white') }
        }
